# Retry: write files again, this time using triple-single-quoted strings to avoid accidental string termination.
from pathlib import Path
import difflib

modified = '''# starwalk_ui.py 
# Streamlit 1.38+
...
# Original content from the user message
original = '''# starwalk_ui.py 
# Streamlit 1.38+
...
if not _HAS_OPENAI:
    ...
    st.session_state.setdefault("qa_messages", [])
    if st.session_state.get("qa_messages") and len(st.session_state["qa_messages"]) > 8:
        st.session_state["qa_messages"] = st.session_state["qa_messages"][-8:]

    # Show last few exchanges; older hidden behind an expander
    older = st.session_state["qa_messages"][:-2]
    latest = st.session_state["qa_messages"][-2:]
    if older:
        with st.expander("Show previous Q&A"):
...
Path("/mnt/data/starwalk_ui_modified.py").write_text(modified, encoding="utf-8")

diff_lines = difflib.unified_diff(
    original.splitlines(keepends=True),
    modified.splitlines(keepends=True),
    fromfile="starwalk_ui.py (original)",
    tofile="starwalk_ui.py (modified)",
    n=3,
)
Path("/mnt/data/starwalk_ui_patch.diff").write_text("".join(diff_lines), encoding="utf-8")

print("Created files:")
print(" - /mnt/data/starwalk_ui_modified.py")
print(" - /mnt/data/starwalk_ui_patch.diff")
