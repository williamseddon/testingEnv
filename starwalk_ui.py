...
@media(max-width:768px){.hero-grid{grid-template-columns:1fr;}.summary-grid{grid-template-columns:1fr;}.builder-card{padding:14px 14px 15px;}.hero-title{font-size:20px;}.metric-card{min-height:94px;}.ref-wrap{display:inline-block;max-width:100%;}.ref-tip{left:0;transform:none;width:min(360px,calc(100vw - 20px));max-height:min(56vh,360px);}.ref-tip-inner{padding:15px 16px 17px 16px;}.ref-item{padding:10px 3px 11px 3px;}}
</style>
        ...
        if source_mode == SOURCE_MODE_URL:
            ...
            if url_mode == "Single link":
                ...
                if st.button("Build combined workspace", type="primary", key="ws_build_url_multi", use_container_width=True):
                    try:
                        nd = _load_multiple_product_reviews(bulk_urls)
                        _reset_review_filters()
                        st.session_state.update(analysis_dataset=nd, chat_messages=[], master_export_bundle=None, prompt_run_artifacts=None, sym_processed_rows=[], sym_new_candidates={}, sym_symptoms_source="none", workspace_active_tab=TAB_DASHBOARD, workspace_tab_request=None, ai_include_references=False, ot_show_volume=False)
                        st.rerun()
                    except requests.HTTPError as exc:
