...
<style>
:root{
  --fg:#0f172a; --muted:#64748b; --bd:#e2e8f0; --card:#ffffff; --app:#f7f9fc;
}
@media (prefers-color-scheme: dark){
  :root{ --fg:#e5e7eb; --muted:#9ca3af; --bd:#334155; --card:#0b1020; --app:#0a0f1b;}
}
...
# -------------------- Fixed containers (prevents jump) --------------------
hdr_ctn    = st.container()
kpi_ctn    = st.container()
pareto_ctn = st.container()
runner_ctn = st.container()
console_ctn= st.container()
review_ctn = st.container()
export_ctn = st.container()
...
def canonicalize(name: str) -> str:
    ALIAS_CANON = {
        ...
        "too loud":"Loud",
        "vacuum sucks":"Poor performance",  # idiom intent
    }
...
def safe_key(prefix: str, *parts) -> str:
    """
    Create unique, stable Streamlit keys from arbitrary parts (handles NaN/None).
    """
...
def verify_json(model: str, system: str, user_obj: dict, force_chat=False) -> dict:
    """
    Calls OpenAI with JSON schema enforcement. Falls back to dict if anything fails.
    """
    cli = safe_client()
    if cli is None:
        return {}
...
def _evidence_score(label: str, text_lower_spaced: str) -> Tuple[int, List[str]]:
    ...
    for t in toks:
        try:
            if re.search(rf"\b{re.escape(t)}\b", text_lower_spaced, flags=re.IGNORECASE):
                hits.append(t)
        except re.error:
            pass
...
# -------------------- Review module (unique keys & undo) --------------------
def render_symptomization_review(df, SYMPTOM_COLS, APP):
    ...
    with review_ctn:
        ...
        with tab_rev:
            ...
            ridx = st.number_input("Row index", min_value=0, max_value=max(0,len(df)-1), value=0, key="row_index_input")
            if 0 <= ridx < len(df):
                ...
                with c1:
                    st.write("**Detractors**")
                    if det:
                        for idx_lab, lab in enumerate(det):
                            lk = str(lab) if lab and str(lab)!="nan" else "<empty>"
                            cc1,cc2,cc3 = st.columns([5,1,1])
                            cc1.write(lk)
                            k_move = safe_key("mv_d_to_del", ridx, lk, idx_lab)
                            k_del  = safe_key("rm_d", ridx, lk, idx_lab)
                            if cc2.button("➡️", key=k_move, help="Move to Delighter"):
                                ...
                                st.success("Moved.")
                            if cc3.button("🗑️", key=k_del, help="Remove label"):
                                cols = [c for c in list(SYMPTOM_COLS) if c in df.columns]
                ...
                with c2:
                    st.write("**Delighters**")
                    if deL:
                        for idx_lab, lab in enumerate(deL):
                            lk = str(lab) if lab and str(lab)!="nan" else "<empty>"
                            cc1,cc2,cc3 = st.columns([5,1,1])
                            cc1.write(lk)
                            k_move = safe_key("mv_l_to_det", ridx, lk, idx_lab)
                            k_del  = safe_key("rm_l", ridx, lk, idx_lab)
                            if cc2.button("➡️", key=k_move, help="Move to Detractor"):
                                cols = list(SYMPTOM_COLS) + [
                                    APP["SAFETY_FLAG_COL"], APP["SAFETY_EVIDENCE_COL"], APP["RELIABILITY_FLAG_COL"],
                                    APP["RELIABILITY_MODE_COL"], APP["RELIABILITY_COMP_COL"], APP["RELIABILITY_SEV_COL"],
                                    APP["RELIABILITY_RPN_COL"], APP["RELIABILITY_QUOTE_COL"], APP["SUGGESTION_SUM_COL"],
...
def main():
    ...
    st.session_state["VIZ_MODE"] = st.sidebar.radio(
    ...
    # Allowed lists (simple UI; you can still wire to sheet later)
    ...
    with runner_ctn:
        ...
        with colQ1:
            queue_all = st.button("Queue ALL", disabled=st.session_state["RUN_LOCKED"], key="queue_all_btn")
        with colQ2:
            sample_n = st.number_input("Or queue first N", 1, max(1,len(df)), min(80,len(df)), key="queue_n_input")
            queue_n = st.button("Queue N", disabled=st.session_state["RUN_LOCKED"], key="queue_n_btn")
        with colQ3:
            st.caption("Adaptive batching keeps latency predictable.")
        ...
        with st.expander("Why it may take time (and what to do)", expanded=False):
            st.markdown(
                "- Long reviews or tight strictness increase compute.\n"
                "- Stay in **Tables** view while processing to keep the UI snappy.\n"
                "- Min LLM confidence around **0.70–0.75** balances precision/recall.\n"
                "- Use **Shortest first** to clear easy wins.\n"
            )

        if remaining:
            st.info(f"Queued rows: {len(queued)} • Remaining: {remaining}")
            prog = st.progress(ptr / len(queued))
            go = st.button(f"Process next {min(BATCH, remaining)}", disabled=st.session_state["RUN_LOCKED"], key="process_next_btn")
            if go:
                ...
                try:
                    ...
                    for ridx in span:
                        ...
                        det = [canonicalize(x) for x in det if canonicalize(x) in allowed_det_set][:10]
                        deL = [canonicalize(x) for x in deL if canonicalize(x) in allowed_del_set][:10]
                    ...
                    try:
                        df_ckpt = df.copy()
                        df_ckpt.to_parquet("starwalk_checkpoint.parquet", index=False)
                    ...
                    st.session_state["RUN_LOCKED"] = False
                    st.error("Processing failed but the app stayed alive.")
        ...
        with col1:
            if fmt_ok and fmt_bytes:
                st.download_button("⬇️ Download (preserve formatting)", data=fmt_bytes,
                                   file_name="starwalk_v12_1_formatted.xlsx",
