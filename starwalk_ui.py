...
if count_max == 0:
    ...
    c1, c2, c3, c4, _ = st.columns([2,1,1,1,1.6])
    with c1:
        n_default = 10 if count_max >= 10 else max(1, count_max)
        n_to_sym_main = st.number_input("How many to symptomize (from top of scope)", min_value=1, max_value=count_max, value=n_default, step=1)
    with c2:
        run_n_main = st.button("Symptomize N", use_container_width=True)
    with c3:
        run_all_main = st.button("Symptomize All", use_container_width=True)
    with c4:
        overwrite_btn = st.button("Overwrite & Re‑symptomize", use_container_width=True)

    if run_n_main or run_all_main or overwrite_btn:
        if client is None:
            ...
            mode_overwrite = bool(overwrite_btn)
            if mode_overwrite:
                df = ensure_ai_columns(df)
                idxs_to_clear = rows_iter.index.tolist()
                for idx_clear in idxs_to_clear:
                    for j in range(1,11):
