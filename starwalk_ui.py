# Write a new version that defaults to *model-native semantics* (no hardcoded negators/synonym lists).
# File: /mnt/data/star_walk_app_v4_native.py

code = r'''# -*- coding: utf-8 -*-
# Star Walk — Symptomize Reviews (v4, Native Semantics)
# Streamlit 1.38+
#
# This version minimizes reliance on hardcoded lexicons:
# - No NEGATORS/ALIAS/SYN_SEEDS in native mode (default ON).
# - Negation + paraphrase detection is delegated to the model.
# - Evidence is verified via (a) quote presence OR (b) label↔sentence embedding similarity.
# - Variant expansion is disabled by default; you can re-enable if needed.
#
# To run:
#   pip install streamlit openpyxl openai pandas numpy
#   export OPENAI_API_KEY=YOUR_KEY
#   streamlit run star_walk_app_v4_native.py
...
# ---------------- Page Config ----------------
st.set_page_config(layout="wide", page_title="Star Walk Analysis Dashboard (Native)")
...
st.markdown(
    ...
    <div class="hero-wrap">
      <div class="hero-inner">
        <div>
          <div class="hero-title">Star Walk — Symptomize Reviews (v4, Native Semantics)</div>
          <div class="hero-sub">No hand-tuned negators/synonyms — rely on model + embeddings.</div>
        </div>
...
with st.sidebar:
    ...
    # Native semantics (disable hand lexicons)
    native_semantics = st.checkbox("Model-native semantics (no lexicons)", value=True)
    variant_boost = st.checkbox("(Optional) Enable synonym/variant expansion", value=False, help="Off by default to avoid hand-tuned lists.")
    ...
    st.info("Evidence acceptance: quote present in the review OR label↔sentence embedding similarity ≥ threshold.")
...
# ---- Candidate shortlist (embeddings-first, no variants in native mode) ----
...
def _llm_pick(review: str, stars, allowed_del: list[str], allowed_det: list[str], min_conf: float,
              ...
              native_semantics: bool, variant_boost: bool, auto_relax: bool, ensemble_check: bool):
    ...
    allowed_del_f = _prefilter_candidates(review, allowed_del, candidate_cap, client, emb_model, max_sentences, native_semantics, variant_boost)
    allowed_det_f = _prefilter_candidates(review, allowed_det, candidate_cap, client, emb_model, max_sentences, native_semantics, variant_boost)
    ...
    cache_key = "|".join([
        ...
        "native" if native_semantics else "legacy",
...
Rules:
- Use your own reasoning for synonyms/paraphrases (no assistance from a lexicon).
...
- Prefer precision over recall; remove near-duplicates. Max 10 per group. Confidence ∈ [0,1].
        """
    )
    ...
    def _post_process(items, allowed_set, text):
        ...
        for d in items or []:
            ...
            if require_evidence:
                ev_ok = False
                # (a) exact quote fragment present
                if quote and _normalize_name(quote) in _normalize_name(text):
                    ev_ok = True
                # (b) OR embedding similarity exceeds threshold
                if not ev_ok:
                    sim = _best_sim(name)
    ...
    # Conservative fallback (no API): lexical only with simple evidence
    text = " " + _normalize_name(review) + " "
    ...
    dels_f = pick_from_allowed(allowed_del_f)
    dets_f = pick_from_allowed(allowed_det_f)
...
def _render_live_table():
    ...
    if not rows:
        ...
        return
    live_df = pd.DataFrame(rows, columns=["Row #", "Stars", "Len", "Detractors #", "Delighters #", "Status"])
    live_table_ph.dataframe(live_df, use_container_width=True, hide_index=True, height=320)
...
if (run or run_all) and missing_idx:
    ...
    def _process_one(idx: int):
        ...
        return idx, _llm_pick(
            review_txt, stars,
            allowed_del=[s for s in []],  # keep delighters in columns 11-20, using UI palette below
            allowed_det=[s for s in []],  # we will load from Symptoms sheet beneath
            min_conf=strictness,
    ...
    with st.status("Processing reviews...", expanded=True) as status_box:
        ...
        # NOTE: we fill allowed lists here (so they can be edited above if needed)
        # The ALLOWED_* lists are extracted later in this file (from the Symptoms sheet).
        # To avoid code reordering, we fetch them from globals at runtime.
        global ALLOWED_DELIGHTERS, ALLOWED_DETRACTORS
        allowed_del_global = ALLOWED_DELIGHTERS if 'ALLOWED_DELIGHTERS' in globals() else []
        allowed_det_global = ALLOWED_DETRACTORS if 'ALLOWED_DETRACTORS' in globals() else []

        def _process_one_bound(idx: int):
            row = df.loc[idx]
            review_txt = str(row.get("Verbatim", "") or "").strip()
            stars = row.get("Star Rating", None)
            return idx, _llm_pick(
                review_txt, stars,
                allowed_del=allowed_del_global,
                allowed_det=allowed_det_global,
                min_conf=strictness,
                require_evidence=require_evidence,
                evidence_sim_threshold=evidence_sim_threshold,
                candidate_cap=candidate_cap,
        ...
        with ThreadPoolExecutor(max_workers=api_concurrency) as ex:
            futures = {ex.submit(_process_one_bound, idx): idx for idx in todo}
            for fut in as_completed(futures):
                idx = futures[fut]
                try:
                    idx, (dels, dets, _, _, evidence_map) = fut.result()
                    try:
...
# ---------------- After-run UI (review/approve) ----------------
# (We extract the allowed palettes here, so globals are available for the run above)

# Load symptom dictionary/palette after main sheet is known
import io as _io
...
def load_symptom_lists_robust(raw_bytes: bytes, user_sheet: str | None = None, user_del_col: str | None = None, user_det_col: str | None = None):
    ...
    if best_del or best_det:
        ...
        return dels, dets, {"sheet":sheet, "strategy":"fuzzy-headers","columns":list(s.columns)}

    # Fallback: first two non-empty
    non_empty_cols = []
    for c in s.columns:
    ...
    if non_empty_cols:
        ...
        return dels, dets, {"sheet":sheet,"strategy":"first-two-nonempty","columns":list(s.columns)}
    return [], [], {"sheet":sheet,"strategy":"none","columns":list(s.columns)}
...
ALLOWED_DELIGHTERS, ALLOWED_DETRACTORS, SYM_META = load_symptom_lists_robust(
    raw_bytes, user_sheet=chosen_sheet if sheet_names else None, user_del_col=None, user_det_col=None
)
...
with colD:
    st.caption("Native semantics mode: negation/paraphrases handled by the model.")
...
path = "/mnt/data/star_walk_app_v4_native.py"
