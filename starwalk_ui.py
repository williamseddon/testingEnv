# starwalk_ui_v5_fixed.py — Clean, robust, with Submit buttons and type-safe editors
...
# ------------------- Page Setup -------------------
st.set_page_config(layout="wide", page_title="Star Walk Review Analyzer — Fixed")
st.title("🌟 Star Walk Review Analyzer — Fixed")
st.caption("Exact export (K–T dets, U–AD dels) • New-symptom approvals (with submit button) • Polished UI")
...
# Optional meta columns after AD (kept if present in DF)
...
def generate_template_workbook_bytes(
    ...
    """Return workbook bytes with K–T (dets), U–AD (dels), and optional meta (AE/AF/AG) written."""
...
if processed_rows:
    ...
    for rec in processed_rows:
        ...
        with st.expander(head):
            ...
            st.write(rec["Verbatim"])  # full text (no truncation)
...
# Aggregate across processed rows
...
# Suppress near-duplicates against whitelist and aliases
...
def _filter_near_dupes(cmap: Dict[str, List[int]], cutoff: float = 0.94) -> Tuple[Dict[str, List[int]], int]:
    ...
    for sym, refs in cmap.items():
        ...
        try:
            ...
            pass
        if c in seen:
            filtered[seen[c]].extend(refs)
        else:
            filtered[sym] = list(refs)
            seen[c] = sym
    return filtered, suppressed

cand_del, _ = _filter_near_dupes(cand_del)
cand_det, _ = _filter_near_dupes(cand_det)
...
if cand_del or cand_det:
    ...
    def _mk_table(cmap: Dict[str, List[int]], side_label: str) -> pd.DataFrame:
        ...
        # enforce dtypes (prevents Streamlit type-compat errors)
    ...
    # FORM with explicit submit button — fixes the error
