                    ...
                    if det:
                        for lab in det:
                            ...
                            if cc2.button("➡️", key=k1, help="Move to Delighter"):
                                ...
                                # clear all instances of lab in Symptom 1-20
                                for j in range(1,21):
                                    c=f"Symptom {j}"
                                    if c in df.columns and str(df.at[ridx,c]).strip()==lab: df.at[ridx,c]=""
                                # place into first free delighter slot
                                for j in range(11,21):
                                    c=f"Symptom {j}"
        ...
        with col1:
            if fmt_ok and fmt_bytes:
                st.download_button("⬇️ Download (preserve formatting)", data=fmt_bytes,
                                   file_name="starwalk_v12_formatted.xlsx",
