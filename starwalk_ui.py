# starwalk_ui_v4.py — Best-in-class UI + AE/AF/AG Meta (Safety/Reliability/Ownership)
# Streamlit App — Dynamic Symptoms • Clean UI • Model Selector • Smart Auto‑Symptomization
# Approval Inbox with examples • Exact Excel export (K–T dets, U–AD dels) + AE/AF/AG meta • Color‑Coded
# Notes:
...
# Minimal, consistent styling (chips/tiles)
st.markdown(
    """
    <style>
      ...
      .chip.blue {background:#e7f0fb; border-color:#b9d0ff;}
      .chip.yellow {background:#fff7d6; border-color:#ffe08a;}
      .chip.purple {background:#f4e6f8; border-color:#d7b3e6;}
    </style>
    """,
...
with right:
    overwrite_slots = st.toggle("Overwrite template slots for processed rows", value=False, help="If on, clears K–T, U–AD and AE/AF/AG for processed rows before writing new values.")
...
model_label = st.sidebar.selectbox("Model", list(MODEEL_CHOICES.keys()) if False else list(MODEL_CHOICES.keys()), index=1)
