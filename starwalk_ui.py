# starwalk_ui_v7_2.py — sticky toolbar fix, instant presets, clear LLM gating
...
# ------------------- Page -------------------
st.set_page_config(layout="wide", page_title="Review Symptomizer — v7.2")
st.title("✨ Review Symptomizer — v7.2")
...
st.markdown(
    ...
    <style>
      ...
      /* Sticky toolbar */
      .toolbar { position: sticky; top: 8px; z-index: 5; background: rgba(255,255,255,.92);
        backdrop-filter: blur(6px); border: 1px solid #e6eaf0; border-radius: 12px;
        padding: 12px; box-shadow: 0 4px 14px rgba(16,24,40,.06); }
    </style>
    """,
...
def get_symptom_whitelists(file_bytes: bytes) -> Tuple[List[str], List[str], Dict[str, List[str]]]:
    ...
    POS = {"delighter", "delighters", "positive", "pos", "pros"}
    NEG = {"detractor", "detractors", "negative", "neg", "cons"}
    ...
    if label_col and type_col:
        ...
        delighters = _clean(df_sym.loc[df_sym[type_col].isin(POS), label_col])
        detractors = _clean(df_sym.loc[df_sym[type_col].isin(NEG), label_col])
        if alias_col:
...
def detect_symptom_columns(df: pd.DataFrame) -> Dict[str, List[str]]:
    ...
    return {"manual_detractors": man_det, "manual_delighters": man_del, "ai_detractors": ai_det, "ai_delighters": ai_del}
...
def _openai_labeler(verbatim: str, client, model: str, temperature: float,
                    ...
                    del_map: Dict[str, str], det_map: Dict[str, str],
                    alias_to_label: Dict[str, str]) -> Tuple[List[str], List[str], List[str], List[str]]:
    if not verbatim or not verbatim.strip():
        return [], [], [], []
    sys = (
        "Classify this consumer review into delighters and detractors. "
        "Use ONLY the provided whitelists; DO NOT invent labels. "
        "If a synonym is close but not listed, put it under the appropriate 'unlisted' bucket.\n"
    ...
    try:
        ...
        def _map_side(items: List[str], side: str) -> List[str]:
            ...
            for x in (items or []):
                key = _canon(x)
                label = (del_map.get(key) or alias_to_label.get(key)) if side == "del" else (det_map.get(key) or alias_to_label.get(key))
                if label:
                    valid = (label in delighters) if side == "del" else (label in detractors)
                    if valid and label not in mapped:
                        mapped.append(label)
            return mapped[:10]
...
# ------------------- Export -------------------
def generate_template_workbook_bytes(original_file, updated_df: pd.DataFrame,
                                     processed_idx: Optional[Set[int]] = None,
                                     overwrite_processed_slots: bool = False) -> bytes:
...
def add_new_symptoms_to_workbook(original_file, selections: List[Tuple[str, str]]) -> bytes:
    ...
    _ensure_header("Aliases", ["aliases", "alias"], preferred_index=3)
...
st.markdown(
    f"""
    <div class="hero"><div class="hero-stats">
...
scope = st.selectbox("Choose scope",
                     ["Missing both", "Any missing", "Missing delighters only", "Missing detractors only"], index=0)
...
if "n_to_process" not in st.session_state:
    st.session_state["n_to_process"] = 10 if len(target) >= 10 else max(1, len(target))

st.markdown("<div class='toolbar'>", unsafe_allow_html=True)
tb_left, tb_right = st.columns([3,7])

with tb_left:
    st.number_input("How many to symptomize (from top of scope)",
                    min_value=1, max_value=max(1, len(target)),
                    value=st.session_state["n_to_process"], step=1, key="n_to_process_input")
    p1, p2, p3, p4 = st.columns(4)
    if p1.button("10", key="preset_10"):
        st.session_state["n_to_process"] = min(10, max(1, len(target))); st.experimental_rerun()
    if p2.button("25", key="preset_25"):
        st.session_state["n_to_process"] = min(25, max(1, len(target))); st.experimental_rerun()
    if p3.button("50", key="preset_50"):
        st.session_state["n_to_process"] = min(50, max(1, len(target))); st.experimental_rerun()
    if p4.button("100", key="preset_100"):
        st.session_state["n_to_process"] = min(100, max(1, len(target))); st.experimental_rerun()

with tb_right:
    b1, b2, b3, b4, b5 = st.columns(5)
    run_n_btn = b1.button("Symptomize N", key="btn_run_n", use_container_width=True)
    run_all_btn = b2.button("Symptomize All", key="btn_run_all", use_container_width=True)
    overwrite_btn = b3.button("Overwrite & Re-symptomize", key="btn_overwrite", use_container_width=True)
    run_missing_both_btn = b4.button("✨ Missing-Both One-Click", key="btn_missing_both", use_container_width=True)
    undo_btn = b5.button("↩️ Undo last run", key="btn_undo", use_container_width=True)
st.markdown("</div>", unsafe_allow_html=True)

# Keep the input value in state (single source of truth)
st.session_state["n_to_process"] = int(st.session_state.get("n_to_process_input", st.session_state["n_to_process"]))
...
def _run_symptomize(rows_df: pd.DataFrame, overwrite_mode: bool = False):
    if client is None:
        st.error("OpenAI is not configured. Set OPENAI_API_KEY (secrets or env) and install `openai` to run symptomization.")
        return
    ...
    if overwrite_mode:
        ...
        for idx_clear in rows_df.index.tolist():
    ...
    for k, (idx, row) in enumerate(rows_df.iterrows(), start=1):
        ...
        try:
            dels, dets, unl_dels, unl_dets = _openai_labeler(vb, client, selected_model, temperature,
        ...
        try:
            safety, reliability, sessions = _openai_meta_extractor(vb, client, selected_model, temperature)
        except Exception:
...
# Button actions
if run_n_btn or run_all_btn or overwrite_btn or run_missing_both_btn:
...
# ------------------- Processed Reviews (evidence + meta chips) -------------------
if processed_rows:
    ...
    for rec in processed_rows:
        ...
        with st.expander(head):
            ...
            for lab in list(rec["Added_Detractors"]) + list(rec["Added_Delighters"]):
                terms.append(lab)
                if ALIASES and lab in ALIASES: terms.extend(ALIASES[lab])
...
if cand_del or cand_det:
    ...
    def _mk_table(cmap: Dict[str, List[int]], side_label: str) -> pd.DataFrame:
        ...
        for sym, refs in sorted(cmap.items(), key=lambda kv: (-len(kv[1]), kv[0])):
            examples = []
            for ridx in refs[:3]:
                try: examples.append(str(df.loc[ridx, "Verbatim"]))
                except Exception: pass
            rows_tbl.append({
                "Add": False, "Label": sym, "Side": side_label,
                "Count": int(len(refs)),
    ...
    with st.form("new_symptom_candidates_form", clear_on_submit=False):
        ...
        with cA:
            ...
            editor_del = st.data_editor(
                tbl_del, num_rows="fixed", use_container_width=True,
                column_config={
        ...
        with cB:
            ...
            editor_det = st.data_editor(
                tbl_det, num_rows="fixed", use_container_width=True,
                column_config={
                    "Add": st.column_config.CheckboxColumn(),
                    "Label": st.column_config.TextColumn(),
                    "Side": st.column_config.SelectboxColumn(options=["Delighter","Detractor"]),
            ...
            if isinstance(editor_del, pd.DataFrame) and not editor_del.empty:
                for _, r_ in editor_del.iterrows():
                    if bool(r_.get("Add", False)) and str(r_.get("Label", "")).strip():
                        selections.append((str(r_["Label"]).strip(), str(r_.get("Side","Delighter")).strip() or "Delighter"))
        except Exception: pass
        try:
            if isinstance(editor_det, pd.DataFrame) and not editor_det.empty:
                for _, r_ in editor_det.iterrows():
                    if bool(r_.get("Add", False)) and str(r_.get("Label", "")).strip():
                        selections.append((str(r_["Label"]).strip(), str(r_.get("Side","Detractor")).strip() or "Detractor"))
        except Exception: pass

        if selections:
            updated_bytes = add_new_symptoms_to_workbook(uploaded_file, selections)
            st.download_button("⬇️ Download 'Symptoms' (updated)", data=updated_bytes,
...
export_bytes = generate_template_workbook_bytes(uploaded_file, df,
                                                processed_idx=processed_idx_set if processed_idx_set else None,
                                                overwrite_processed_slots=False)

st.download_button("⬇️ Download symptomized workbook (XLSX)",
                   data=export_bytes,
...
with st.expander("📘 View Symptoms from Excel Workbook", expanded=False):
    ...
    def _esc2(s: str) -> str: return str(s).replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
    def _chips(items, color: str):
        items_sorted = sorted({str(x).strip() for x in (items or []) if str(x).strip()})
        if not items_sorted: st.write("(none)")
        else:
            st.markdown("<div class='chip-wrap'>" + "".join([f"<span class='chip {color}'>{_esc2(x)}</span>" for x in items_sorted]) + "</div>", unsafe_allow_html=True)
    ...
    with tabs[3]:
        ...
        with c1:
            st.markdown("**Safety**"); df_s = _count("AI Safety", SAFETY_ENUM)
            st.bar_chart(df_s.set_index("Value")["Count"])
            chips = "<div class='chip-wrap'>" + "".join([f"<span class='chip yellow'>{_esc2(v)} · {int(c)}</span>" for v, c in df_s.itertuples(index=False)]) + "</div>"
            st.markdown(chips, unsafe_allow_html=True)
        with c2:
            st.markdown("**Reliability**"); df_r = _count("AI Reliability", RELIABILITY_ENUM)
            st.bar_chart(df_r.set_index("Value")["Count"])
            chips = "<div class='chip-wrap'>" + "".join([f"<span class='chip blue'>{_esc2(v)} · {int(c)}</span>" for v, c in df_r.itertuples(index=False)]) + "</div>"
            st.markdown(chips, unsafe_allow_html=True)
        with c3:
            st.markdown("**# of Sessions**"); df_n = _count("AI # of Sessions", SESSIONS_ENUM)
            st.bar_chart(df_n.set_index("Value")["Count"])
            chips = "<div class='chip-wrap'>" + "".join([f"<span class='chip purple'>{_esc2(v)} · {int(c)}</span>" for v, c in df_n.itertuples(index=False)]) + "</div>"
            st.markdown(chips, unsafe_allow_html=True)
...
def _label_counts(df_in: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
    ...
    if not vc.empty: vc.columns = ["Label", "Count"]
    return vc
...
if not qp_options:
    ...
    for c in side_cols:
        if c in df.columns:
            try: mask_any = mask_any | (df[c].astype(str).str.strip() == qp_label)
            except Exception: pass
    ...
    try:
        ...
        mention_hits = df.loc[verb_mask & (~mask_any)]
    except Exception:
...
# Footer
st.divider()
st.caption("Exports write EXACTLY to K–T (dets) and U–AD (dels); meta to AE/AF/AG. Undo enabled. Approvals use a real submit button. Sticky toolbar presets now update instantly.")
