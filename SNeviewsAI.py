...
SCRAPE_MODE_OPTIONS = ["Auto", "Fast only", "Recovery only"]
...
def get_secret(name: str) -> str:
    try:
        ...
        for section in ("openai", "bazaarvoice"):
...
def get_bazaarvoice_config() -> Dict[str, str]:
    keys = [
        "BAZAARVOICE_PASSKEY",
        "passkey",
        "BAZAARVOICE_API_VERSION",
        "api_version",
        "BAZAARVOICE_LOCALE",
        "locale",
    ]
    values = {key: get_secret(key) for key in keys}
    return {
        "passkey": values.get("BAZAARVOICE_PASSKEY") or values.get("passkey") or "",
        "api_version": values.get("BAZAARVOICE_API_VERSION") or values.get("api_version") or "5.4",
        "locale": values.get("BAZAARVOICE_LOCALE") or values.get("locale") or "en_US",
    }
...
def extract_candidate_product_ids(html_text: str, page_url: str, ld_meta: Optional[Dict[str, Any]] = None) -> List[str]:
    candidates: List[str] = []
    ld_meta = ld_meta or {}
    ...
    for node in soup.find_all(True):
        for attr in ('data-bv-product-id', 'data-product-id', 'data-sku', 'data-item-no', 'data-itemno', 'data-item-number'):
            add(node.get(attr))
    ...
    regexes = [
        ...
        r"[\"']product_id[\"']\s*[:=]\s*[\"']([^\"']+)[\"']",
        r"[\"']sku[\"']\s*[:=]\s*[\"']([^\"']+)[\"']",
        r'Item\s*No\.?\s*</[^>]+>\s*<[^>]+>\s*([A-Z0-9_-]{3,})',
        r'Item\s*No\.?\s*[:#]?\s*([A-Z0-9_-]{3,})',
    ]
    ...
    for piece in re.split(r'[/_-]+', parsed.path):
        if MODEL_CODE_RE.fullmatch(piece or ''):
            add(piece.upper())
...
def extract_candidate_passkeys(html_text: str, request_urls: Sequence[str]) -> List[str]:
    candidates: List[str] = []
    regexes = [
    ...
    for url in request_urls:
        for match in re.findall(r'[?&]passkey=([A-Za-z0-9]+)', url, flags=re.I):
            candidates.append(match)
    seen = set()
    ordered: List[str] = []
    for value in candidates:
...
def build_bazaarvoice_api_candidates(
    page_url: str,
    html_text: str,
    ld_meta: Optional[Dict[str, Any]],
    discovered_urls: Sequence[str],
    preferred_passkeys: Sequence[str] = (),
    api_version: str = '5.4',
    locale: str = 'en_US',
) -> List[str]:
    parsed = urlparse(page_url)
    ...
    for url in discovered_urls:
        ...
        if candidate_host and 'bazaarvoice' in candidate_host.lower():
            api_hosts.append(candidate_host)
    if not api_hosts:
        api_hosts = [
            'api.bazaarvoice.com',
            'display.ugc.bazaarvoice.com',
            f'{host}.ugc.bazaarvoice.com' if host else '',
        ]
    product_ids = extract_candidate_product_ids(html_text, page_url, ld_meta)
    passkeys = list(preferred_passkeys) + extract_candidate_passkeys(html_text, discovered_urls)
    candidates: List[str] = []
    for api_host in api_hosts:
        if not api_host:
            continue
        base = f'https://{api_host}/data/reviews.json'
        for passkey in passkeys or ['']:
            for product_id in product_ids:
                for filter_value in (f'ProductId:{product_id}', f'ProductId:eq:{product_id}'):
                    query = {
                    ...
                    candidates.append(replace_query_params(base, query))
    seen = set()
    ordered: List[str] = []
...
def preflight_bazaarvoice_product_ids(
    product_ids: Sequence[str],
    passkey: str,
    api_version: str,
    locale: str,
) -> List[str]:
    if not passkey or not product_ids:
        ...
        try:
            ...
            if isinstance(parsed, dict):
                for result in parsed.get('Results', []) or []:
                    stats = result.get('ProductStatistics') or result
                    for block_name in ('ReviewStatistics', 'NativeReviewStatistics'):
                        block = stats.get(block_name) or {}
                        candidate_total = block.get('TotalReviewCount')
                        if isinstance(candidate_total, (int, float)):
                            total = max(total, int(candidate_total))
        except Exception:
            total = -1
        scored.append((total, -idx, product_id))
    scored.sort(reverse=True)
    positives = [product_id for total, _, product_id in scored if total > 0]
    if positives:
        trailing = [product_id for total, _, product_id in scored if total <= 0]
        return positives + trailing
    return list(product_ids)
...
def rank_api_urls(request_urls: Sequence[str]) -> List[str]:
    ranked = []
    for idx, url in enumerate(request_urls):
        lowered = url.lower()
        score = 0
    ...
    return [url for score, _, url in ranked if score > 0]
...
def fetch_reviews_from_bazaarvoice_api(
    ...
    candidate_bases = rank_api_urls(api_urls)
    for base_url in candidate_bases:
        urls_to_try = [replace_query_params(base_url, {'Limit': str(limit), 'Offset': '0'})]
        if preferred_sort:
            urls_to_try.insert(0, replace_query_params(base_url, {'Limit': str(limit), 'Offset': '0', 'Sort': preferred_sort}))
        for initial_url in urls_to_try:
            ...
            try:
                ...
                batch_rows = dedupe_reviews(batch_rows)
                if not batch_rows:
                    continue
                rows = batch_rows
    ...
    if not rows:
        return [], raw_payloads, {'api_url': None, 'total_results': None, 'pages_fetched': 0, 'attempted_urls': attempted_urls}
    ...
    while len(dedupe_reviews(rows)) < min(target_reviews, total_results):
        ...
        next_url = replace_query_params(used_url, {"Limit": str(limit), "Offset": str(offset)})
        try:
...
def build_manual_api_candidates(manual_feed_url: str, manual_product_id: str, manual_passkey: str, product_url: str, api_version: str = "5.4", locale: str = "en_US") -> List[str]:
    candidates: List[str] = []
    if manual_feed_url:
        candidates.append(manual_feed_url.strip())
    if manual_product_id and manual_passkey:
        base = "https://api.bazaarvoice.com/data/reviews.json"
        query = {
            "apiversion": "5.5",
            "passkey": manual_passkey.strip(),
            "Filter": f"ProductId:{manual_product_id.strip()}",
            "Limit": "100",
            "Offset": "0",
            "Sort": SORT_TO_BAZAARVOICE.get("recent", "SubmissionTime:desc"),
        }
        candidates.append(base + "?" + urlencode(query))
    deduped: List[str] = []
    for value in candidates:
...
def capture_rendered_page_with_playwright(
    product_url: str,
    progress_cb: Callable[[str, str, float, int], None],
    ...
    try:
        ...
        raise RuntimeError(
            "Recovery mode needs the optional Playwright browser helper. Install it with `pip install playwright` and then run `python -m playwright install chromium`."
        ) from exc

    progress_cb("Recovery browser", "Opening a lightweight browser session to observe the live review requests...", 0.78, 0)
    ...
    with sync_playwright() as p:
        ...
        def on_response(response: Any) -> None:
            try:
                url = response.url
                lowered = url.lower()
                if any(token in lowered for token in ["bazaarvoice", "reviews.json", "review", "powerreviews"]):
    ...
    ranked = rank_api_urls(captured_urls)
    progress_cb("Recovery browser", f"Browser recovery captured {len(ranked)} likely review request(s).", 0.84, 0)
    return rendered_html, final_url, ranked


def scrape_sharkninja_reviews_hybrid(
    product_url: str,
    target_reviews: int,
    sort_key: str,
    progress_cb: Callable[[str, str, float, int], None],
    ...
    manual_passkey: str = "",
    bazaarvoice_passkey: str = "",
    bazaarvoice_api_version: str = "5.4",
    bazaarvoice_locale: str = "en_US",
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Dict[str, Any]]:
    rows, raw_payloads, meta = scrape_sharkninja_reviews(
        ...
        manual_feed_url=manual_feed_url,
        manual_product_id=manual_product_id,
        manual_passkey=manual_passkey,
        bazaarvoice_passkey=bazaarvoice_passkey,
        bazaarvoice_api_version=bazaarvoice_api_version,
        bazaarvoice_locale=bazaarvoice_locale,
    )
    return rows, raw_payloads, meta
...
def scrape_sharkninja_reviews(
    ...
    candidate_product_ids = extract_candidate_product_ids(html_text, normalized_url, ld_meta)
    preferred_passkey = manual_passkey.strip() or bazaarvoice_passkey.strip()
    ranked_product_ids = preflight_bazaarvoice_product_ids(
        candidate_product_ids,
        preferred_passkey,
        bazaarvoice_api_version,
        bazaarvoice_locale,
    )
    meta["candidate_product_ids"] = ranked_product_ids
    discovered_api_urls = extract_candidate_review_api_urls(html_text, normalized_url)
    constructed_api_urls = build_bazaarvoice_api_candidates(
        normalized_url,
        html_text,
        ld_meta,
        discovered_api_urls,
        preferred_passkeys=[preferred_passkey] if preferred_passkey else [],
        api_version=bazaarvoice_api_version,
        locale=bazaarvoice_locale,
    )
    candidate_api_urls = []
    for value in list(discovered_api_urls) + list(constructed_api_urls):
        if value not in candidate_api_urls:
            candidate_api_urls.append(value)
    for product_id in ranked_product_ids:
        for value in build_manual_api_candidates(
            manual_feed_url="",
            manual_product_id=product_id,
            manual_passkey=preferred_passkey,
            product_url=normalized_url,
            api_version=bazaarvoice_api_version,
            locale=bazaarvoice_locale,
        ):
            if value not in candidate_api_urls:
                candidate_api_urls.insert(0, value)
    for value in build_manual_api_candidates(
        manual_feed_url,
        manual_product_id,
        preferred_passkey,
    ...
    if candidate_api_urls:
        progress_cb(
            'Fast review feed',
            f'Found {len(candidate_api_urls)} possible review feed path(s). Trying the fastest direct feed options first...',
            0.42,
            len(merged_seed),
        )
        api_rows, api_raw, api_meta = fetch_reviews_from_bazaarvoice_api(
            api_urls=candidate_api_urls,
            product_url=normalized_url,
            target_reviews=target_reviews,
            sort_key=sort_key,
            progress_cb=progress_cb,
    ...
    should_try_recovery = scrape_mode in {"Auto", "Recovery only"} and len(merged) < min(target_reviews, 25)
    if should_try_recovery:
        try:
            rendered_html, recovery_final_url, recovery_urls = capture_rendered_page_with_playwright(normalized_url, progress_cb)
            normalized_url = normalize_url(recovery_final_url)
            recovery_ld = extract_ld_reviews(extract_ld_json_objects(rendered_html), normalized_url)
            recovery_dom = parse_reviews_from_dom_html(rendered_html, normalized_url)
            recovery_candidates = []
            for value in recovery_urls + extract_candidate_review_api_urls(rendered_html, normalized_url):
                if value not in recovery_candidates:
                    recovery_candidates.append(value)
            for value in build_manual_api_candidates(manual_feed_url, manual_product_id, preferred_passkey, normalized_url, api_version=bazaarvoice_api_version, locale=bazaarvoice_locale):
                if value not in recovery_candidates:
                    recovery_candidates.insert(0, value)
            recovery_api_rows, recovery_raw, recovery_meta = fetch_reviews_from_bazaarvoice_api(
                api_urls=recovery_candidates,
                product_url=normalized_url,
                target_reviews=target_reviews,
                sort_key=sort_key,
                progress_cb=progress_cb,
            )
            raw_payloads.extend(recovery_raw)
            merged = dedupe_reviews(merged + recovery_ld + recovery_dom + recovery_api_rows)[:target_reviews]
            meta["recovery_used"] = True
            meta["recovery_attempted_api_urls"] = recovery_meta.get("attempted_urls", [])
            if recovery_meta.get("api_url"):
                meta["captured_api_url"] = recovery_meta.get("api_url")
        except Exception as recovery_exc:
            meta["recovery_used"] = False
            meta["recovery_error"] = str(recovery_exc)

    if not merged:
        observed = []
        if meta.get('site_review_count') is not None:
            observed.append(f"site_review_count={meta.get('site_review_count')}")
        if ld_rows:
            observed.append(f"ld_json={len(ld_rows)}")
        if dom_rows:
            observed.append(f"dom={len(dom_rows)}")
        if api_meta.get('attempted_urls'):
            observed.append(f"feed_candidates={len(api_meta.get('attempted_urls', []))}")
        if manual_product_id:
            observed.append(f"manual_product_id={manual_product_id}")
        if meta.get("recovery_error"):
            observed.append("recovery_failed")
        diagnostic = '; '.join(observed) if observed else 'no review signals found in page HTML or feed candidates'
        raise RuntimeError(
            'No reviews could be extracted from this SharkNinja page. '
            f'Diagnostics: {diagnostic}. Try Recovery mode or add a manual product id / feed override for this SKU.'
        )
    ...
    meta["captured_api_url"] = meta.get("captured_api_url") or api_meta.get("api_url")
    meta["captured_request_count"] = len(candidate_api_urls)
    meta["attempted_api_urls"] = api_meta.get("attempted_urls", [])
    meta["scrape_mode"] = scrape_mode
    meta["manual_feed_url"] = bool(manual_feed_url)
    meta["manual_product_id"] = manual_product_id or ""
...
def main() -> None:
    ...
    with st.sidebar:
        bv_config = get_bazaarvoice_config()
        st.subheader("OpenAI")
        ...
        if use_secrets and bv_config.get("passkey"):
            st.caption("Bazaarvoice passkey loaded from Streamlit secrets.")
        else:
            st.caption("No Bazaarvoice passkey detected in secrets. Add [bazaarvoice].passkey or BAZAARVOICE_PASSKEY for direct API retrieval.")
        ...
        scrape_mode = st.selectbox("Scrape mode", options=SCRAPE_MODE_OPTIONS, index=0, help="Auto starts browserless and only uses browser recovery if coverage is still thin. Fast only stays browserless. Recovery only jumps straight to live browser-assisted capture.")
        accept_sharkclean = st.toggle("Allow SharkClean URLs that redirect to SharkNinja", value=True)
        with st.expander("Manual review feed override", expanded=False):
            manual_product_id = st.text_input("Manual product id", value="", help="Use this when you already know the Bazaarvoice product id or SKU feeding reviews.")
            manual_passkey = st.text_input("Manual Bazaarvoice passkey", value=bv_config.get("passkey", "") if use_secrets and bv_config.get("passkey") else "", help="Optional. Pair with a product id to construct the feed directly. If left alone, the app uses the Bazaarvoice passkey from Streamlit secrets when available.")
            manual_feed_url = st.text_input("Manual feed URL", value="", help="Optional full review feed URL. This is tried before auto-discovered candidates.")
    ...
    product_url = st.text_input(
        ...
        placeholder="https://www.sharkclean.com/products/shark-navigator-lift-away-adv-with-self-cleaning-brushroll-zidLA362",
        help="Use a direct public product detail page. Auto mode now tries embedded reviews, discovered review feeds, constructed Bazaarvoice paths, and optional browser recovery when the browserless path is too thin.",
    )

    normalized_url = normalize_url(product_url)
    domain_hint = urlparse(normalized_url).netloc if normalized_url else ""
    status_cols = st.columns([1.1, 0.9, 0.9, 1.1], vertical_alignment="center")
    ...
    status_cols[3].caption(f"Mode: {scrape_mode}")
    ...
    if scrape_clicked:
        if not normalized_url:
            ...
            try:
                rows, raw_payloads, meta = scrape_sharkninja_reviews_hybrid(
                    product_url=normalized_url,
                    target_reviews=target_reviews,
                    sort_key=SORT_OPTIONS[sort_label],
                    progress_cb=progress_cb,
                    scrape_mode=scrape_mode,
                    manual_feed_url=manual_feed_url.strip(),
                    manual_product_id=manual_product_id.strip(),
                    manual_passkey=manual_passkey.strip(),
                    bazaarvoice_passkey=(manual_passkey.strip() or (bv_config.get("passkey", "") if use_secrets else "")),
                    bazaarvoice_api_version=bv_config.get("api_version", "5.4"),
                    bazaarvoice_locale=bv_config.get("locale", "en_US"),
                )
